ruleset manage_sensors {
    meta {
        ...
        shares nameFromID, showChildren, sensors, all_temperatures, subscriptions, sensor_list, reportData, lastFiveReports
    }
    ...
    global {
        ...
        lastFiveReports = function() {
            low_index = (ent:report_number - 5) < 0 =>  0 | ent:report_number - 5
            ent:report_data.keys().slice(low_index, ent:report_number - 1).map(function(key){
                {}.put(key, ent:report_data{key})
            }).reduce(function(a, b) {
                a.put(b.keys().head(), b.values().head())
            })
        }
    }
    ...
    rule sensor_report {
        ...
        pre {
            rcn = ent:report_number;
        }
        always {
            raise sensor event "scatter_report_request"
                attributes {
    ...
    rule scatter_request {
        select when sensor scatter_report_request
        foreach sensors() setting(sensor_info)
        pre {
            rcn = event:attrs{"report_correlation_number"}
            sensor_eci = sensor_info{"Tx"}
            request_source_eci = sensor_info{"Rx"}
            expected_response_count = sensors().length()
        }
        if not rcn.isnull() then
            event:send({
                ...
                "attrs": {
                    "report_correlation_number": rcn,
                    "request_source_eci": request_source_eci
                }
            })
        fired {
            ent:report_data{rcn} := {
                "temperature_sensors": expected_response_count,
                "responding": 0,
                "temperatures": []
            }
        }
    ...
    rule catch_sensor_response {
        select when sensor sensor_report_reponse
        pre {
            rcn = event:attrs{"report_correlation_number"}
            temperature = event:attrs{"temperature"}
            updated = (ent:report_data{[rcn, "temperatures"]}).defaultsTo([]).append(temperature)
        }
        if not rcn.isnull() && not temperature.isnull() then noop()
        fired {
            ent:report_data{[rcn, "temperatures"]} := updated
            ent:report_data{[rcn, "responding"]} := ent:report_data{[rcn, "temperatures"]}.length()
        }
    }
}